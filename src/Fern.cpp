...
bool Ferns::addFrame(DeviceArray<float>& imageTexture, DeviceArray2D<float>& vertexTexture, DeviceArray2D<float>& normalTexture,
  ...
  resizev(vertexTexture, vertexResized);
  resizev(normalTexture, normalResized);
...
Eigen::Matrix4f Ferns::findFrame(std::vector<SurfaceConstraint>& constraints, const Eigen::Matrix4f& currPose, DeviceArray2D<float>&  vertexTexture,
